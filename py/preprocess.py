...
def add_nfl_pbp_info(
    ...
    if len(gpid_without_passer) > 0:
        LOG.info('Defaulting passer to QB for %d plays without a passer', len(gpid_without_passer))
        qbs = (
            tracking.query('position == "QB" and gpid in @gpid_without_passer')
            [['gpid','nfl_id']].drop_duplicates()
        )
        qbs = qbs.rename(columns={'nfl_id': 'passer_nfl_id'})
        tracking = tracking.merge(qbs, on='gpid', how='left')
